...
<template>
  <UPage v-if="page">
    ...
    >
      ...
      >
        <div class="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div class="grid gap-4 text-sm text-muted sm:grid-cols-2 lg:min-w-[22rem]">
            <div>
    ...
    <UPageSection
      v-if="page.invite"
      :title="page.invite.title"
      :description="page.invite.description"
      :ui="{
        container: '!pt-0'
      }"
    >
      <UCard
        class="border border-dashed border-default bg-elevated/40"
        :ui="{
          body: 'flex flex-col gap-4 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8'
        }"
      >
        <p class="max-w-2xl text-sm text-muted">
          This page will grow into a speaking archive over time, but it starts with the next real talk instead of a manufactured history.
        </p>
        <UButton
          v-if="page.invite.link"
          v-bind="page.invite.link"
        />
      </UCard>
    </UPageSection>
  </UPage>
